...
const origin = require("../models/originCountries");

db.on("error", console.error.bind(console, "MongoDB Connection Error!"));

const main = async () => {
  const originCountry1 = await new origin({
    name: "South Africa",
    country_flag:
  ...
  const originCountry2 = await new origin({
    name: "Pakistan",
    country_flag:
      "https://upload.wikimedia.org/wikipedia/commons/3/32/Flag_of_Pakistan.svg",
    motto: "ایمان، اتحاد، نظ",
  });
  originCountry2.save();

  const originCountry3 = await new origin({
    name: "Japan",
    country_flag:
  ...
  const originCountry4 = await new origin({
    name: "France",
    country_flag:
  ...
  const originCountry5 = await new origin({
    name: "Chile",
    country_flag:
      "https://upload.wikimedia.org/wikipedia/commons/7/78/Flag_of_Chile.svg",
    motto: "Por la razón o la fuerzaé",
  });
  originCountry5.save();
